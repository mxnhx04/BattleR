  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/70 backdrop-blur-sm px-4 py-6">
      <div className="w-full max-w-sm bg-brand-charcoal border border-white/10 rounded-xl p-5 max-h-[80vh] overflow-y-auto">
        <div className="font-heading font-bold text-2xl tracking-wide mb-4 text-center">
          {title}
        </div>
        <div className="flex flex-col gap-2">
          {targets.map((t) => (
            <button
              ...
              className="flex items-center justify-between gap-3 rounded-lg border border-white/10 bg-white/[0.03] px-4 py-3 text-left hover:border-brand-gold hover:bg-brand-gold/10 transition-colors"
            >
